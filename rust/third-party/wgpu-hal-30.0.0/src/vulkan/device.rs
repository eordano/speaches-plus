...
impl super::Device {
    ...
    fn compile_stage(
        ...
        let required_subgroup_size = if naga_stage == naga::ShaderStage::Compute
            && self.shared.features.contains(wgt::Features::SUBGROUP)
        {
            std::env::var("NV_WGPU_REQUIRED_SUBGROUP_SIZE")
                .ok()
                ...
                .map(|w| {
                    Box::new(
                        vk::PipelineShaderStageRequiredSubgroupSizeCreateInfo::default()
                            .required_subgroup_size(w),
                    )
                })
        } else {
            None
        };
        if let Some(info) = required_subgroup_size.as_deref() {
            create_info.flags &= !vk::PipelineShaderStageCreateFlags::ALLOW_VARYING_SUBGROUP_SIZE;
            // Same lifetime circumvention as p_name above: the box outlives create_info.
            let info: *const vk::PipelineShaderStageRequiredSubgroupSizeCreateInfo = info;
            create_info.p_next = info.cast();
        }
