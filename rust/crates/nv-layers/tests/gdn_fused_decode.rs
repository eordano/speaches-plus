...
fn run_shape(name: &str, n_k: usize, n_v: usize, d_k: usize, d_v: usize) {
    ...
    for (i, (r, f)) in ref_outs.iter().zip(fused_outs.iter()).enumerate() {
        let err = rel_l2(f, r);
        eprintln!("[{name}] step {i} rel_l2(fused vs candle) = {err:.3e}");
        assert!(
            err < 2e-2,
            "[{name}] fused decode diverged at step {i}: {err}"
        );
    }
}

#[test]
fn gdn_fused_decode_tiny_shape() {
    run_shape("tiny", 2, 4, 32, 32);
}

#[test]
fn gdn_fused_decode_qwen36_shape() {
    run_shape("qwen36", 16, 32, 128, 128);
}

#[test]
fn gdn_fused_decode_qwen38_shape_v_per_k_3_48v_16k_128() {
    run_shape("qwen38", 16, 48, 128, 128);
}

#[test]
fn gdn_fused_decode_tiny_v_per_k_3_discriminates_group_ratio_from_head_count() {
    run_shape("tiny_vpk3", 2, 6, 32, 32);
}
