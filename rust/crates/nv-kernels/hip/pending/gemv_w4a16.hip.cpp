...
__global__ void gemv_w4a16_kernel(
    ...
    const __hip_bfloat16* s_row = scale + (size_t)n * groups;

    float acc = 0.0f;

    if (kUseShared && GS >= 32 && Kv <= 4 * kRowLanes) {
        uint4 pw[4];
        float sc[4];
        int cnt = 0;
        for (int v = lane; v < Kv; v += kRowLanes) {
            pw[cnt] = w_row[v];
            sc[cnt] = __bfloat162float(s_row[(v * 32) / GS]);
            ++cnt;
        }
        int i = 0;
        for (int v = lane; v < Kv; v += kRowLanes) {
...
__global__ void gemv_w4a16_row_kernel(
    ...
    constexpr int kGroups = kThreads / kRowLanes;
    ...
    const uint4* w_row = reinterpret_cast<const uint4*>(packed + (size_t)n * Kw);
    const __hip_bfloat16* s_row = scale + (size_t)n * groups;
    const __hip_bfloat162* x2 = reinterpret_cast<const __hip_bfloat162*>(x);
    ...
    for (int v = tid; v < Kv; v += kThreads) {
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            #pragma unroll
            for (int i = 0; i < 4; ++i) {
                float2 xf = __bfloat1622float2(x2[(kb >> 1) + i]);
    ...
    acc = row_group_sum(acc);
    __shared__ float group_sums[kGroups];
    if (lane == 0) group_sums[grp] = acc;
    __syncthreads();
    if (grp == 0) {
        float sum = (lane < kGroups) ? group_sums[lane] : 0.0f;
        sum = row_group_sum(sum);
...
__global__ void gemv_w4a16_gelu_pli_kernel(
    ...
    float sc[4];
    int cnt = 0;
    for (int v = lane; v < Kv && cnt < 4; v += kRowLanes) {
        pw[cnt] = w_row[v];
        sc[cnt] = __bfloat162float(s_row[(v * 32) / GS]);
        ++cnt;
    }
    ...
    for (int v = lane; v < Kv; v += kRowLanes) {
        if (i < 4) {
            ...
            uint4 w = w_row[v];
            float s2 = __bfloat162float(s_row[(v * 32) / GS]);
...
extern "C" int nv_kernels_gemv_w4a16_gelu_pli(
    ...
    gemv_w4a16_gelu_pli_kernel<<<grid, dim3(kBlockDim), 0, s>>>(
        packed,
        reinterpret_cast<const __hip_bfloat16*>(scale),
...
extern "C" int nv_kernels_gemv_w4a16(
    ...
    if (N <= 0 || K <= 0 || GS <= 0) return 0;
    if ((K & 31) != 0 || (K % GS) != 0 || (GS & 7) != 0) return -1;
    hipStream_t s = (hipStream_t)stream;
    dim3 grid((unsigned)((N + kRowsPerBlock - 1) / kRowsPerBlock));
