...
__device__ __forceinline__ float row_group_sum(float v) {
    #pragma unroll
    for (int o = kRowLanes / 2; o > 0; o >>= 1) {
        v += __shfl_xor(v, o, kRowLanes);
    }
    return v;
}
...
__device__ __forceinline__ float dot32(const uint4& pw, const float* xp) {
    ...
    for (int j = 0; j < 4; ++j) {
        ...
        #pragma unroll
        for (int i = 0; i < 4; ++i) {
            float fe = ubyte_lane_f32(lo, i) - 8.0f;
            float fo = ubyte_lane_f32(hi, i) - 8.0f;
            a = fmaf(fe, x8[2 * i], a);
...
__global__ void gemv_w4a16_m1_proto_kernel(
    ...
    constexpr int kBlockDim = kWarps * kRowLanes;
    ...
    if (n < N) {
        ...
        const __hip_bfloat16* s_row = scale + (size_t)n * Kv;

        constexpr int kStride = kRowLanes * kSplit;
        int v0 = part * kRowLanes + lane;
...
int launch_proto(
    ...
    constexpr int kRows = kWarps / kSplit;
    int Kv = K >> 5;
    int needed = (Kv + kRowLanes * kSplit - 1) / (kRowLanes * kSplit);
    dim3 grid((unsigned)((N + kRows - 1) / kRows));
    dim3 block(kWarps * kRowLanes);
    size_t smem = (size_t)Kv * 33 * sizeof(float);

    int lds_cap = max_lds_bytes();
    if (lds_cap <= 0 || smem > (size_t)lds_cap) return -2;
...
extern "C" int nv_kernels_gemv_w4a16_m1_proto(
    ...
    const __hip_bfloat16* sc = reinterpret_cast<const __hip_bfloat16*>(scale);
    const __hip_bfloat16* xb = reinterpret_cast<const __hip_bfloat16*>(x);
    __hip_bfloat16* yb = reinterpret_cast<__hip_bfloat16*>(y);
    switch (variant) {
