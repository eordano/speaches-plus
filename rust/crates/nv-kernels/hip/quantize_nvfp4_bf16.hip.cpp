...
__device__ __forceinline__ uint8_t encode_ue4m3_dev(float scale) {
    ...
    int frexp_e = 0;
    float frac = frexpf(clamped, &frexp_e);
    int exp_v = frexp_e - 1;
...
__device__ __forceinline__ float decode_ue4m3_dev(uint8_t b) {
    ...
    return ldexpf(1.f + mant / 8.f, exp_v - 7);
}
...
__global__ void quantize_nvfp4_bf16_kernel(
    ...
    int blocks_ceil4 = (blocks_per_row + 3) & ~3;
    bool row_in_range = (row < m_logical);
    bool data_row = (row < m_padded);
    const __hip_bfloat16* xrow = row_in_range ? (x + (size_t)row * K) : nullptr;
    uint8_t* prow = data_row ? (packed_out + (size_t)row * (K / 2)) : nullptr;

    float stored = (stored_global == 0.f || !isfinite(stored_global)) ? 1.f : stored_global;

    for (int kb = threadIdx.x; kb < blocks_ceil4; kb += blockDim.x) {
        if (!row_in_range || kb >= blocks_per_row) {

            if (data_row && kb < blocks_per_row) {
                uint8_t* pblock = prow + kb * 8;
                #pragma unroll
                for (int i = 0; i < 8; ++i) pblock[i] = 0;
            }
            scales_out[swizzled_scale_dst(row, kb, blocks_per_row)] = 0;
            continue;
        }
        float vals[16];
        float amax = 0.f;
...
__global__ void quantize_nvfp4_bf16_per_expert_kernel(
    ...
    int blocks_per_row = K / 16;
    const __hip_bfloat16* xrow = x + (size_t)row * K;
...
__global__ void silu_mul_quantize_nvfp4_bf16_per_expert_kernel(
    const __hip_bfloat16* __restrict__ y_gate,
    const __hip_bfloat16* __restrict__ y_up,
    uint8_t* __restrict__ packed_out,
    uint8_t* __restrict__ scales_out,
    const float* __restrict__ stored_globals,
    int m_per_expert,
    ...
    if (stored == 0.f || !isfinite(stored)) stored = 1.f;
    int blocks_per_row = K / 16;
    const __hip_bfloat16* g_row = y_gate + (size_t)row * K;
    const __hip_bfloat16* u_row = y_up   + (size_t)row * K;
    uint8_t* prow = packed_out + (size_t)row * (K / 2);
...
extern "C" int nv_kernels_silu_mul_quantize_nvfp4_bf16_per_expert(
    ...
    silu_mul_quantize_nvfp4_bf16_per_expert_kernel<<<m_total, 256, 0, s>>>(
        reinterpret_cast<const __hip_bfloat16*>(y_gate_bf16),
        reinterpret_cast<const __hip_bfloat16*>(y_up_bf16),
        packed_out, scales_out_swizzled,
        stored_globals, m_per_expert, m_total, K
    );
...
extern "C" int nv_kernels_quantize_nvfp4_bf16(
    ...
    int sf_rows = ((m_padded + 127) / 128) * 128;
    dim3 grid(sf_rows);
    dim3 block(256);
