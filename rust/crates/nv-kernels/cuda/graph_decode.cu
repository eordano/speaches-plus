...
__device__ inline float gd_block_sum(float v) {
    constexpr int kW = 32;
    constexpr int kWarps = BLOCK / kW;
    __shared__ float warp_sums[kWarps];
    __shared__ float total;
    int lane = threadIdx.x & (kW - 1);
    int warp = threadIdx.x / kW;
    #pragma unroll
    for (int o = kW / 2; o > 0; o >>= 1) v += __shfl_xor_sync(0xffffffffu, v, o);
...
__global__ void cast_bf16_f32_kernel(const __nv_bfloat16* __restrict__ x,
...
__global__ void rms_no_weight_kernel(const __nv_bfloat16* __restrict__ x,
                                     float* __restrict__ y, int rows, int dim,
                                     float eps) {
    int r = blockIdx.x;
    if (r >= rows) return;
    int tid = threadIdx.x;
    const __nv_bfloat16* xr = x + (size_t)r * dim;
...
__global__ void gelu_mul_bf16f32_kernel(const __nv_bfloat16* __restrict__ gate,
...
__global__ void cast_scale_bf16_f32_kernel(const __nv_bfloat16* __restrict__ x,
...
extern "C" int nv_kernels_cast_scale_bf16_f32(void* stream, const uint16_t* x, float* y,
    ...
    cast_scale_bf16_f32_kernel<<<blocks, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(x), y, scale, n);
...
extern "C" int nv_kernels_add_scale_f32(void* stream, const float* a, const float* b,
    ...
    if (n <= 0) return 0;
    int blocks = (n + kBlock - 1) / kBlock;
    add_scale_f32_kernel<<<blocks, kBlock, 0, (cudaStream_t)stream>>>(a, b, y, scale, n);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_cast_bf16_f32(void* stream, const uint16_t* x, float* y, int n) {
    if (n <= 0) return 0;
    int blocks = (n + kBlock - 1) / kBlock;
    cast_bf16_f32_kernel<<<blocks, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(x), y, n);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_cast_f32_bf16(void* stream, const float* x, uint16_t* y, int n) {
    if (n <= 0) return 0;
    int blocks = (n + kBlock - 1) / kBlock;
    cast_f32_bf16_kernel<<<blocks, kBlock, 0, (cudaStream_t)stream>>>(
        x, reinterpret_cast<__nv_bfloat16*>(y), n);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_rms_no_weight_bf16_f32(void* stream, const uint16_t* x,
    ...
    rms_no_weight_kernel<<<(unsigned)rows, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(x), y, rows, dim, eps);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_gelu_mul_bf16f32(void* stream, const uint16_t* gate,
    ...
    if (n <= 0) return 0;
    int blocks = (n + kBlock - 1) / kBlock;
    gelu_mul_bf16f32_kernel<<<blocks, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(gate), pli,
        reinterpret_cast<__nv_bfloat16*>(y), n);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_argmax_f32_rows(
    ...
    if (rows > 65535) return NVK_ERR_GRID_AXIS;
    dim3 g1(kArgmaxBlocks, rows);
    argmax_f32_rows_stage1_kernel<<<g1, kBlock, 0, (cudaStream_t)stream>>>(
        logits, n, part_val, part_idx);
    argmax_f32_rows_stage2_kernel<<<rows, kArgmaxBlocks, 0, (cudaStream_t)stream>>>(
...
__global__ void rmsnorm_add_scale_bf16_kernel(
    ...
    int tid = threadIdx.x;
    const __nv_bfloat16* xr = x + (size_t)r * dim;
    const __nv_bfloat16* rr = res + (size_t)r * dim;
    __nv_bfloat16* yr = y + (size_t)r * dim;
    float partial = 0.0f;
    for (int d = tid; d < dim; d += kBlock) {
        float v = __bfloat162float(xr[d]);
        partial += v * v;
    ...
    for (int d = tid; d < dim; d += kBlock) {
        ...
        __nv_bfloat16 ob = __float2bfloat16(out);
        yr[d] = ob;
    ...
    if (rstd_out != nullptr || normed_out != nullptr) {
        ...
        if (normed_out != nullptr) {
            __nv_bfloat16* nr = normed_out + (size_t)r * dim;
            for (int d = tid; d < dim; d += kBlock) {
...
__global__ void qkv_prep_kernel(
    ...
    const __nv_bfloat16* xr = qkv + (size_t)head * HD;
    ...
    const __nv_bfloat16* w = (kind == 0) ? qw : (kind == 1 ? kw : nullptr);
    for (int d = tid; d < HD; d += kBlock) {
...
__global__ void rstd_bf16_kernel(const __nv_bfloat16* __restrict__ x,
...
__global__ void rms_apply_bf16_kernel(const __nv_bfloat16* __restrict__ x,
                                      const __nv_bfloat16* __restrict__ w,
...
extern "C" int nv_kernels_incr_pos_rope(void* stream, int* pos, int* rope_pos) {
    incr_pos_rope_kernel<<<1, 1, 0, (cudaStream_t)stream>>>(pos, rope_pos);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_argmax_bf16(
    ...
    if (n <= 0) return -1;
    argmax_bf16_stage1_kernel<<<kArgmaxBlocks, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(logits), n, part_val, part_idx);
    argmax_bf16_stage2_kernel<<<1, kArgmaxBlocks, 0, (cudaStream_t)stream>>>(
...
extern "C" int nv_kernels_token_map_u32(
    ...
    token_map_u32_kernel<<<1, 1, 0, (cudaStream_t)stream>>>(map, idx, out);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_rmsnorm_bf16w_f32out(void* stream, const uint16_t* x,
    ...
    if (rows <= 0 || dim <= 0) return 0;
    rmsnorm_bf16w_f32out_kernel<<<(unsigned)rows, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(x),
        reinterpret_cast<const __nv_bfloat16*>(w), y, rows, dim, eps);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_rmsnorm_add_scale_bf16(void* stream, const uint16_t* x,
    ...
    rmsnorm_add_scale_bf16_kernel<<<(unsigned)rows, kBlock, smem, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(x),
        reinterpret_cast<const __nv_bfloat16*>(w),
        reinterpret_cast<const __nv_bfloat16*>(res),
        reinterpret_cast<__nv_bfloat16*>(y), rstd_out,
        reinterpret_cast<const __nv_bfloat16*>(next_w),
        reinterpret_cast<__nv_bfloat16*>(normed_out), rows, dim, eps, scale, eps_next);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_qkv_prep(void* stream, const uint16_t* qkv,
    ...
    qkv_prep_kernel<<<(unsigned)blocks, kBlock, 0, (cudaStream_t)stream>>>(
        ...
        reinterpret_cast<__nv_bfloat16*>(kcache),
        reinterpret_cast<__nv_bfloat16*>(vcache), NH, NKV, HD, eps);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_rstd_bf16(void* stream, const uint16_t* x, float* rstd_out,
                                    int rows, int dim, float eps) {
    if (rows <= 0 || dim <= 0) return 0;
    rstd_bf16_kernel<<<(unsigned)rows, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(x), rstd_out, rows, dim, eps);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_rms_apply_bf16(void* stream, const uint16_t* x,
    ...
    if (n <= 0) return 0;
    int blocks = (n + kBlock - 1) / kBlock;
    rms_apply_bf16_kernel<<<blocks, kBlock, 0, (cudaStream_t)stream>>>(
        reinterpret_cast<const __nv_bfloat16*>(x),
        reinterpret_cast<const __nv_bfloat16*>(w), rstd,
        reinterpret_cast<__nv_bfloat16*>(y), n, dim);
    return (int)cudaGetLastError();
}

extern "C" int nv_kernels_incr_pos(void* stream, int* pos) {
    incr_pos_kernel<<<1, 1, 0, (cudaStream_t)stream>>>(pos);
    return (int)cudaGetLastError();
}
...
extern "C" int nv_kernels_write_kv_f32(
    ...
    write_kv_f32_kernel<<<(unsigned)NKV, kBlock, 0, (cudaStream_t)stream>>>(
...
extern "C" int nv_kernels_attn_decode_dev_f32(
    ...
    attn_decode_dev_kernel<<<(unsigned)NH, kBlock, shmem, (cudaStream_t)stream>>>(
...
extern "C" int nv_kernels_multi_zero_bf16(void* stream, const void* list, int n) {
    ...
    multi_zero_bf16_kernel<<<(unsigned)n, 256, 0, (cudaStream_t)stream>>>(
