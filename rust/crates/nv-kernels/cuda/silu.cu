...
template <>
__device__ inline float silu_to_f32<__nv_bfloat16>(__nv_bfloat16 x) { return __bfloat162float(x); }
...
template <>
__device__ inline __nv_bfloat16 silu_from_f32<__nv_bfloat16>(float x) { return __float2bfloat16(x); }
...
extern "C" int nv_kernels_silu_bf16(
    ...
    int grid = (int)((n + BLOCK - 1) / BLOCK);
    if (grid <= 0) return 0;
    silu_kernel<__nv_bfloat16><<<grid, BLOCK, 0, s>>>(
...
extern "C" int nv_kernels_silu_mul_bf16(
    ...
    int grid = (int)((n + BLOCK - 1) / BLOCK);
    if (grid <= 0) return 0;
    silu_mul_kernel<__nv_bfloat16><<<grid, BLOCK, 0, s>>>(
        reinterpret_cast<const __nv_bfloat16*>(x),
...
__global__ void silu_mul_bf16_candle_kernel(const __nv_bfloat16* __restrict__ x,
                                            const __nv_bfloat16* __restrict__ gate,
                                            __nv_bfloat16* __restrict__ y,
                                            size_t n) {
    size_t idx = (size_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
    __nv_bfloat16 v = x[idx];
    __nv_bfloat16 s = v / (static_cast<__nv_bfloat16>(1) + hexp(-v));
    y[idx] = s * gate[idx];
}

extern "C" int nv_kernels_silu_mul_bf16_candle(
    void* stream,
    const uint16_t* x,
    const uint16_t* gate,
    uint16_t* y,
    ...
    int grid = (int)((n + BLOCK - 1) / BLOCK);
    if (grid <= 0) return 0;
    silu_mul_bf16_candle_kernel<<<grid, BLOCK, 0, s>>>(
        reinterpret_cast<const __nv_bfloat16*>(x),
        reinterpret_cast<const __nv_bfloat16*>(gate),
        reinterpret_cast<__nv_bfloat16*>(y),
        n);
