...
// Standalone (no-torch, no-pybind) vendoring of vLLM's csrc/core/scalar_type.hpp.
// Source: https://raw.githubusercontent.com/vllm-project/vllm/main/csrc/core/scalar_type.hpp
//
// The only external dependency in the upstream header is STD_TORCH_CHECK (from
// <torch/headeronly/util/Exception.h>). It is replaced below with a standalone
// macro that is valid in constexpr contexts (throwing in a constexpr function is
// only evaluated when the condition is actually false, which never happens for
// the well-formed constants defined here). Everything else (member names,
// signatures, constexpr logic) is byte-for-byte faithful so that the other
// vendored Marlin headers compile against it unchanged.
...
// Faithful enough stand-in for torch's check macro: aborts/throws when the
// condition is false. Usable inside constexpr functions (the throw is only
// reached on a false condition, which is never the case for the constants here).
#define STD_TORCH_CHECK(cond, ...) \
  ((cond) ? (void)0 : throw std::runtime_error("STD_TORCH_CHECK failed: " #cond))
#endif
...
// ---------------------------------------------------------------------------
// Additional faithful aliases for names referenced by some Marlin kernel
// headers / call sites. These do not exist verbatim in the upstream header but
// alias the equivalent upstream constants so vendored kernels compile unchanged.
// ---------------------------------------------------------------------------
static inline constexpr auto kFE2M1fn = kFE2M1f;            // NAN_NONE e2m1
static inline constexpr auto kFloat4_e2m1fn = kFE2M1f;      // NVFP4 element type
static inline constexpr auto kInt = kS8;                    // generic signed int
static inline constexpr auto kUint = kU8;                   // generic unsigned int
