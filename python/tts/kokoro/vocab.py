...
VOCAB: dict[str, int] = {
    ...
    "ː": 158, "ʰ": 162, "ʲ": 164, "↓": 169, "→": 171, "↗": 172,
    "↘": 173, "ᵻ": 177,
}

_PHONEME_REPLACEMENTS: tuple[tuple[str, str], ...] = (
    ("kəkˈoːɹoʊ", "kˈoʊkəɹoʊ"),
    ("kəkˈɔːɹəʊ", "kˈəʊkəɹəʊ"),
)

_PHONEME_CHAR_MAP: dict[str, str] = {
    "ʲ": "j",
    "r": "ɹ",
    "x": "k",
    "ɬ": "l",
...
def clean_phonemes(phonemes: str) -> str:
    out = phonemes
    for needle, replacement in _PHONEME_REPLACEMENTS:
        out = out.replace(needle, replacement)
    out = "".join(_PHONEME_CHAR_MAP.get(c, c) for c in out)
    return "".join(c for c in out if c in VOCAB).strip()
