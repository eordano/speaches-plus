...
from .configuration_qwen3_tts import (Qwen3TTSSpeakerEncoderConfig)
...
class Res2NetBlock(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        outputs = []
        for i, chunk in enumerate(torch.chunk(hidden_states, self.scale, dim=1)):
            if i == 0:
                processed = chunk
            elif i == 1:
                processed = self.blocks[i - 1](chunk)
            else:
                processed = self.blocks[i - 1](chunk + processed)
...
class SqueezeExcitationBlock(nn.Module):
    ...
    def forward(self, hidden_states):
        channel_gate = hidden_states.mean(dim=2, keepdim=True)
        channel_gate = self.relu(self.conv1(channel_gate))
        channel_gate = self.sigmoid(self.conv2(channel_gate))
        return hidden_states * channel_gate
...
class AttentiveStatisticsPooling(nn.Module):
    ...
    def _compute_statistics(self, features, weights, dim=2):
        mean = (weights * features).sum(dim)
        std = torch.sqrt((weights * (features - mean.unsqueeze(dim)).pow(2)).sum(dim).clamp(self.eps))
        return mean, std
